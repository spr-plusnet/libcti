 ...
 *
 */
public class CTIConnectionImpl implements CTIConnection {

	private final static Logger logger = LogManager.getLogger("connector.cti");
	 ...
	 * @throws IOException 
	 */
	public CTIConnectionImpl(Properties config) throws IOException {
		eventChannelByName = new HashMap<String, EventChannelImpl>();
		this.config   = config;
		try {
			if (!config.containsKey(CTIDriver.PROP_XSI_APPNAME)) {
				config.put(CTIDriver.PROP_XSI_APPNAME, "libxsi run by "+System.getProperty("user.name")+" using Java "+System.getProperty("java.version")+" on "+InetAddress.getLocalHost().getHostAddress());
			}
		} catch (IOException e) {
		}

		// Add eventually necessary config properties
		encoding = config.getProperty(CTIDriver.PROP_XSI_ENCODING,"UTF-8");
		useSSL   = Boolean.parseBoolean(config.getProperty(CTIDriver.PROP_XSI_USESSL,"true"));
		useCType = Boolean.parseBoolean(config.getProperty(CTIDriver.PROP_XSI_USECTYPE,"false"));

		hostport = config.getProperty(CTIDriver.PROP_XSI_SERVER);
		if (hostport==null) 
			throw new IllegalArgumentException("Missing property "+CTIDriver.PROP_XSI_SERVER+" in configuration");

		user = config.getProperty(CTIDriver.PROP_XSI_USER);
		pass = config.getProperty(CTIDriver.PROP_XSI_PASS);
		if (user==null) 
			throw new IllegalArgumentException("Missing property "+CTIDriver.PROP_XSI_USER+" in configuration");
//		if (pass==null) 
//			throw new IllegalArgumentException("Missing property "+CTIDriver.PROP_CTI_PASS+" in configuration");
...
//		services.put(ServiceType.SERVICES, new ServicesService(this));
		...
		logger.debug("CTIConnection("+config+") called");
	}
	...
	//-----------------------------------------------------------------
	/**
	 * @see com.broadsoft.xsi.api.CTIConnection#getUser()
	 */
	@Override
	...
	//-----------------------------------------------------------------
	/**
	 * @see com.broadsoft.xsi.api.CTIConnection#actionGETQuery(java.lang.String)
	 */
	@Override
	public Object actionGETQuery(String subURL) throws IOException {

		try {
			URL url = new URL(String.format("http"+(useSSL?"s":"")+"://%s/com.broadsoft.cti-actions/v2.0/%s", hostport, subURL));
			logger.info("GET "+url);
			...
			if (config.containsKey(CTIDriver.PROP_XSI_APPNAME)) 
				con.setRequestProperty("User-Agent", (String)config.getProperty(CTIDriver.PROP_XSI_APPNAME));
...
//					try { Thread.sleep(20); } catch (Exception e) {}
				...
				try {
					return CTIDriver.unmarshall(message);
	...
	//-----------------------------------------------------------------
	/**
	 * @see com.broadsoft.xsi.api.CTIConnection#actionPUTQuery(java.lang.String, byte[])
	 */
	@Override
	...
	//-----------------------------------------------------------------
	/**
	 * @see com.broadsoft.xsi.api.CTIConnection#actionPUTQuery(java.lang.String, java.lang.Object)
	 */
	@Override
	public <T> Object actionPUTQuery(String subURL, T toEncode) throws IOException {
		...
		try {
			CTIDriver.marshall(toEncode, out);
			out.close();
	...
	//-----------------------------------------------------------------
	/**
	 * @see com.broadsoft.xsi.api.CTIConnection#eventsPOSTQuery(java.lang.String)
	 */
	@Override
...
//				con.getInputStream().read(mess);
	...
	//-----------------------------------------------------------------
	/**
	 * @see com.broadsoft.xsi.api.CTIConnection#actionPOSTQuery(java.lang.String, java.lang.Object)
	 */
	@Override
...
//	//-----------------------------------------------------------------
//	/**
//	 * @see com.broadsoft.xsi.api.CTIConnection#createEventChannel(java.lang.String)
//	 */
//	@Override
...
//	}
	...
	//-----------------------------------------------------------------
	/**
	 * @see com.broadsoft.xsi.api.CTIConnection#close()
	 */
	@Override
	public void close() {
